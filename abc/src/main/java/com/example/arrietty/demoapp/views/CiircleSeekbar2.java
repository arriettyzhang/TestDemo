...
public class CiircleSeekbar2 extends View {
    //弧度转角度
    ...
    public CiircleSeekbar2(Context context) {
        this(context, null);
    }

    public CiircleSeekbar2(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CiircleSeekbar2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttrs(AttributeSet attrs, int defStyleAttr) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.CircleSeekBar2,
                defStyleAttr, 0);
        mCircleLineWidth = a.getDimension(R.styleable.CircleSeekBar2_circle_line_width2, 20);
        mSeekbarBgColor = a.getColor(R.styleable.CircleSeekBar2_seekbar_bg_color2, Color.BLUE);
        mThumbRadius = (int) a.getDimension(R.styleable.CircleSeekBar2_thumb_radius2, mCircleLineWidth / 2);
        mThumbDrawable = a.getDrawable(R.styleable.CircleSeekBar2_thumb_drawable2);
        if (mThumbDrawable == null) {
            mThumbDrawable = getContext().getResources().getDrawable(R.drawable.s2_icon_volum_control);
        }
        mStartAngle = a.getInt(R.styleable.CircleSeekBar2_start_angle2, 135);
        mMaxAngle = a.getInt(R.styleable.CircleSeekBar2_max_angle2, 270);
        mCurrentProgerss = a.getInt(R.styleable.CircleSeekBar2_current_progress2, 0);
        mMaxProgress = a.getInt(R.styleable.CircleSeekBar2_max_progress2, 100);
        a.recycle();
        //处理角度偏差
        mCalculateStartAngle = mStartAngle % 90;
        mThumbRadius = Math.min(mThumbDrawable.getIntrinsicWidth(), mThumbDrawable.getIntrinsicHeight()) / 2;
    }
...
//        int diameter=Math.min(width,height);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mRadius = Math.min(w - getPaddingLeft() - getPaddingRight(), h - getPaddingTop() -
                getPaddingBottom()) / 2;
        circleSeekbarRadius = mRadius - mThumbRadius + (int) mCircleLineWidth;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        mCircleSeekPaint.setStrokeWidth(mCircleLineWidth / 2);
        ...
        Rect drawableRect = calculateThumRect();
        mThumbDrawable.setBounds(drawableRect);
        mThumbDrawable.draw(canvas);
    }

    private Rect calculateThumRect() {
        int left = (int) (mPointX - mThumbRadius - mCircleLineWidth);
        int top = (int) (mPointY - mThumbRadius - mCircleLineWidth);
        int right = (int) (mPointX + mThumbRadius + mCircleLineWidth);
        int bottom = (int) (mPointY + mThumbRadius + mCircleLineWidth);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                calculateProgress();
                if (mListener != null) {
                    mListener.onStartTrackingTouch(CiircleSeekbar2.this);
                }
                break;
            ...
            case MotionEvent.ACTION_UP:
                ...
                calculateProgress();
                if (mListener != null) {
                    mListener.onStopTrackingTouch(CiircleSeekbar2.this);
                }
                break;
            case MotionEvent.ACTION_CANCEL:
                if (mListener != null) {
                    mListener.onCancleTrackingTouch(CiircleSeekbar2.this);
                }
                break;
        }
        invalidate();
    ...
    public interface OnProgressChangeListener {
        ...
        void onStartTrackingTouch(CiircleSeekbar2 CiircleSeekbar2);

        void onStopTrackingTouch(CiircleSeekbar2 CiircleSeekbar2);

        void onCancleTrackingTouch(CiircleSeekbar2 CiircleSeekbar2);
    }
}
